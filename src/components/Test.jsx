...
class Test extends Component {
  ...
  toggle() {
    if (!this.isToggle) {
      this.setState({
        items: [{ key: 'a', data: <Box />, in: true }, { key: 'b', data: <Box />, in: true }], // remove c.
      });
    } else {
      this.setState({
        items: [
  ...
  render() {
    return (
      <div>
        <TransitionScale items={this.state.items} wrapperStyle={wrapperStyle} />
        <HandlerScale items={this.state.items2} in={!this.isToggle} wrapperStyle={wrapperStyle} />
